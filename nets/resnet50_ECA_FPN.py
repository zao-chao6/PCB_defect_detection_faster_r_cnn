...
class ResNet50_ECA_FPN(nn.Module):
    def __init__(self, block, layers, num_classes=100,k_size=[3, 3, 3, 3]):
        #-----------------------------------#
        #   假设输入进来的图片是600,600,3
        #-----------------------------------#
        super(ResNet50_ECA_FPN, self).__init__()
        self.inplanes = 64
...
# test
# FPN = ResNet50_ECA_FPN(Bottleneck, [3, 4, 6, 3])
# print('FPN:',FPN)


def resnet50_ECA_FPN(pretrained=False):
    # 对应resnet50的网络结构shape，第五次压缩是在roi中使用，有3个bottleneck。
    model = ResNet50_ECA_FPN(Bottleneck, [3, 4, 6, 3])
