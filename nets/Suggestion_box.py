...
class FasterRCNNTrainer_fpn(nn.Module):
    def __init__(self, model_train, optimizer):
        super(FasterRCNNTrainer_fpn, self).__init__()
        self.model_train    = model_train
        self.optimizer      = optimizer
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        rpn_loc_loss_all, rpn_cls_loss_all, roi_loc_loss_all, roi_cls_loss_all  = 0, 0, 0, 0
        sample_rois, sample_indexes, gt_roi_locs, gt_roi_labels, gt_roi_locs_k, gt_roi_labels_k = [], [],[], [], [], []
        
        # 对p2~p6每一层特征进行rpn部分生成建议框
        for k in range(0,5):
            # 保存每层建议框结果
            sample_rois_k, sample_indexes_k = [], []
            for i in range(n):
                bbox        = bboxes[i]
                label       = labels[i]
                rpn_loc     = rpn_locs[k][i]
                rpn_score   = rpn_scores[k][i]
                roi         = rois[k][i]
                # -------------------------------------------------- #
                #   利用真实框和先验框获得建议框网络应该有的预测结果
                #   给每个先验框都打上标签
                ...
                #   gt_rpn_label    [num_anchors, ]
                # -------------------------------------------------- #
                gt_rpn_loc, gt_rpn_label = self.anchor_target_creator(bbox, anchor[k][0].cpu().numpy())
                gt_rpn_loc = torch.Tensor(gt_rpn_loc).type_as(rpn_loc)
                gt_rpn_label = torch.Tensor(gt_rpn_label).type_as(rpn_loc).long()
                # -------------------------------------------------- #
                #   分别计算建议框网络的回归损失和分类损失
                # -------------------------------------------------- #
                rpn_loc_loss = self._fast_rcnn_loc_loss(rpn_loc, gt_rpn_loc, gt_rpn_label, self.rpn_sigma)
                ...
                # ------------------------------------------------------ #
                sample_roi, gt_roi_loc, gt_roi_label = self.proposal_target_creator(roi, bbox, label, self.loc_normalize_std)
                sample_rois_k.append(torch.Tensor(sample_roi).type_as(rpn_loc))
                sample_indexes_k.append(torch.ones(len(sample_roi)).type_as(rpn_loc) * roi_indices[k][i][0])
                gt_roi_locs_k.append(torch.Tensor(gt_roi_loc).type_as(rpn_loc))
                gt_roi_labels_k.append(torch.Tensor(gt_roi_label).type_as(rpn_loc).long())
            
            #------------------------------------------------------------------#
            #   获得RPN网络的预测结果，进行格式调整，把五个特征层的结果进行堆叠
            #   torch.cat()两个tensor间会增加现有维度的值,续接，垂直拼接：dim=0（列数相同），水平拼接：dim=1（行数相同）
            #   .stack（）会新加增加一个维度，可以理解为叠加（序列中所有的张量都应该为相同形状。）
            #------------------------------------------------------------------#
            sample_rois.append(torch.stack(sample_rois_k, dim=0)) 
            sample_indexes.append(torch.stack(sample_indexes_k, dim=0))
            
        #  维度整合，从[128，4]到[512，4],共batch_size组。
        for g in range(n):
            gt_roi_locs.append(torch.cat(gt_roi_locs_k[g*4:(g+1)*4], dim=0))
            gt_roi_labels.append(torch.cat(gt_roi_labels_k[g*4:(g+1)*4], dim=0))

        # 进入faster rcnn网络进行多尺度分类预测,
        roi_cls_locs, roi_scores = self.model_train([base_feature, sample_rois, sample_indexes, img_size], mode = 'fpn_head')
        for i in range(n):
            # ------------------------------------------------------ #
            #   根据建议框的种类，取出对应的回归预测结果
            # ------------------------------------------------------ #
